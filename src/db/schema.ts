 ...
 *
 * Mode is part of the identity, not a note: a rate evidenced for a venue's
...
export const shadowFeeTierEvidence = pgTable(
  ...
  {
    /** Generated by the repository — PGlite has no gen_random_uuid default. */
    id: uuid("id").primaryKey(),
    sourceId: text("source_id").notNull(),
    /** ORDER_BOOK | EASY_TRADE | CONVERT | OTC_QUOTE */
    executionMode: text("execution_mode").notNull(),
    /** Null when the evidence names no tier. Never a guessed default. */
    tierLabel: text("tier_label"),
    makerFeeBps: integer("maker_fee_bps"),
    takerFeeBps: integer("taker_fee_bps"),
    provenance: text("provenance").notNull(),
